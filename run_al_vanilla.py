...
from networks.nonlinearnet_aihuman import ClassifierSimple, optimization_loop
...
def do_run(
        seed, 
        init_model_path = "models/classifier_simple.pt",
    ...
    jobname = f"l2d-hitl-demo2_Tanimoto_classifierOnly"
    ...
    l2d_model = ClassifierSimple(d, params["dropout"])
        ...
        if acquisition is not None:            
           ...
           ########################### HITL rounds ######################################
            for t in np.arange(T): # T number of AL iterations
                ...
                X_train = np.concatenate([X_train.numpy(), new_x])
                y_train = np.concatenate([y_train.numpy().reshape(-1), new_h])
                ...
                criterion_classifier = params["criterion_classifier"]
                for epoch in range(num_epochs):
                    optimizer.zero_grad()
                    outs = l2d_model(X_train)

                    # losses for clf 1 and system based on y
                    loss_clf1 = criterion_classifier(outs, y_train)

                    loss_clf1.backward()
                    optimizer.step()

                    if (epoch + 1) % 10 == 0:
                        print(f"Epoch [{epoch + 1}/{num_epochs}], Loss: {loss_clf1.item()}")
