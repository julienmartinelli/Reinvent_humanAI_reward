...
def optimization_loop(num_epochs, optimizer, model, X, X_h, y, h, criterion, active_learning = False):
    for epoch in range(num_epochs):
        ...
        boolean = (y == 1).reshape(-1) * (decision_outputs[:,0] > combined_outputs[:, 0]) * (combined_outputs[:, 1] > combined_outputs[:, 0]) + (y == 0).reshape(-1) * (decision_outputs[:,0] > combined_outputs[:, 0]) * (combined_outputs[:, 1] < combined_outputs[:, 0]) * 1.0
        ...
        boolean_h = (h == 1).reshape(-1) * (decision_outputs_h[:,0] > combined_outputs_h[:, 0]) * (combined_outputs_h[:, 1] > combined_outputs_h[:, 0]) + (h == 0).reshape(-1) * (decision_outputs_h[:,0] > combined_outputs_h[:, 0]) * (combined_outputs_h[:, 1] < combined_outputs_h[:, 0]) * 1.0
        ...
        outputs_h = (
                boolean_h * combined_outputs_h[:, 1] + (1 - boolean_h) * combined_outputs_h[:, 0]
        ).unsqueeze(1)
...
def get_prob_distribution(models, X): # requires running X through multiple forward passes to generate a distribution
...
def get_uncertainty_scores(model, X):
    ...
    for i in range(len(prob1_h)):
        scores.append(
            - np.sum(
                [
                    prob0_h[i] * np.log2(prob0_h[i]), 
                    prob1_h[i] * np.log2(prob1_h[i])
                ]
            )
        )
    return scores
